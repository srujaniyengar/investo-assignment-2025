...
psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f "$(dirname "$0")/create_table.sql"

# Load the data using the \copy command
if [ -f "$(dirname "$0")/../csv/data.csv" ]; then
  echo "Copying data from 'data.csv' into the database..."
  PGPASSWORD="$POSTGRES_PASSWORD" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\copy ticker_data(datetime, close, high, low, open, volume, instrument) FROM '$(dirname "$0")/../csv/data.csv' DELIMITER ',' CSV HEADER;"
  echo "Data copied successfully."
else
  echo "Error: 'data.csv' not found at $(dirname "$0")/../csv/"
  exit 1
fi
